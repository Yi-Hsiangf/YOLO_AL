...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    rand_state = np.random.RandomState(134)
    train_idx = []
    pool_idx = list(range(800)) #len of dataset
    count = 100
    for cycle in range(3):
        save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, Active_learning = \
            Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
            opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.AL
        ...
        train_loader, dataset = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                                ...
                                                prefix=colorstr('train: '), train_idx=train_idx, subset=True)
        ...
        if RANK in [-1, 0]:
            val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
                                        ...
                                        prefix=colorstr('val: '), subset=False)[0]
        ...
        optimizer_lossnet = SGD(lossnet.parameters(), lr=0.001, momentum=0.9, weight_decay=5e-4)
        scheduler_lossnet = lr_scheduler.MultiStepLR(optimizer_lossnet, milestones=[20,30])
        ...
        for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
            ...
            for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
                ...
                #print("feature map: ", len(modules[0]))
        ...
        if AL == True:
            ...
            unlabel_loader = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                                    ...
                                                    workers=workers, image_weights=opt.image_weights, quad=opt.quad,
                                                    prefix=colorstr('train: '), train_idx=pool_idx, subset=True)[0]
